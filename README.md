...
- 全局隐藏会由选择“继承”的 App 真正继承；App 选择“显示”时仍可覆盖全局隐藏。
- 菜单记录会合并完整命令模型与滚动过程中见到的 cell，不再只记录首屏五行；App 设置同时列出标准编辑操作、已观察的动态操作及所有已保存规则引用。
- 同一语义操作只保留第一项，解决 UIKit 暴露两个内部“粘贴”命令时的重复项。
- 行内图标使用固定的 cell 坐标，菜单展示后冻结横向锚点；UIKit 在滚动边界调整隐藏分页容器时不再把整列向左带动。
- 滚动期间每次 cell 布局都会重新压住原生标题与图标，只保留插件的一套标题和图标，避免“粘贴”重影及文字压住图标。

...
- 文本选择菜单保持系统箭头相邻边不动，只在预计越过顶部或底部安全区域时按完整行缩短可视高度；空间充足时沿用 1.0.34 的尺寸和定位。
- 滚动条和纵向回弹改为根据完整内容高度与实际可视高度判断，安全区域只能容纳少于五行时仍可访问全部菜单项。

## 1.0.34 更新

- 滚动热路径不再逐层恢复包装层 frame，只保持 collection view 本身与菜单水平边界一致；中间包装层关闭裁切，避免系统临时缩窄时露出灰底，同时降低滚动布局开销。
- 所有菜单 cell 的自定义 layout 都从 `x=0` 开始，图标统一使用 22pt 翻页留位加 16pt 内容边距，不再根据滚动过程中的临时 cell 位置动态补偿，消除图标和标题左右错位。

## 1.0.33 更新

- 修复 1.0.32 滑动时灰块重新出现、停止后滚动条向右跳：初次布局时缓存实际列表包装层的正确水平位置，之后在每次原生滚动布局开始前只恢复被 UIKit 改动的 `x/width`，不再停止后整体调整 frame，也不修改纵向几何或滚动偏移。

## 1.0.32 更新

- 优化 1.0.31 的滚动宽度修复：拖动和减速期间不再逐帧遍历包装层或写入 frame，停止滚动后只校正一次宽度；菜单本体使用与列表一致的背景色，滚动期间也不会露出右侧灰底。

## 1.0.31 更新

- 修复菜单刚弹出时正常、上下滑动后右侧出现灰色竖块：UIKit 在滚动布局中会恢复给翻页按钮预留的内部包装层宽度，现在每次 collection view 布局完成后只校正菜单和包装层的水平几何，不重新遍历或重画已有菜单项。
...
- 文本选择菜单改回 UIKit 的系统默认指示器，不再隐藏系统箭头或绘制自定义三角形；容器裁切会保留系统指示器及其材质、阴影。
- 优化长菜单上下滑动：稳定布局直接复用，滚动期间不再重复遍历并重绘已有 cell，自定义布局只计算可见区域附近的菜单项。
